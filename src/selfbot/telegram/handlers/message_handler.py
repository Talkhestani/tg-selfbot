...
async def del_command(ctx: CommandContext) -> None:
    ...
    # The result is written onto the command message itself so no new
    # message is created (the trigger message is excluded from deletion).
    if result.deleted == 0 and result.errors == 0:
        await ctx.respond(msg.DEL_NO_MESSAGES)
        return
