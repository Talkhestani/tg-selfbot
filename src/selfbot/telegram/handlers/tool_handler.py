...
async def screenshot_command(ctx: CommandContext) -> None:
    ...
    await ctx.respond(msg.PROCESSING)
    ...
    try:
        ...
        raise CommandError(msg.SCREENSHOT_ERROR.format(exc.__class__.__name__)) from exc
    try:
...
async def swear_command(ctx: CommandContext) -> None:
    import asyncio
    ...
    texts = toolbox.random_swears(count)
    if not texts:
        raise CommandError("فهرست فحش‌ها خالی است")
...
def register(dispatcher, services) -> None:  # type: ignore[no-untyped-def]
    ...
    dispatcher.register(
        CommandSpec(
            name="swear",
            handler=swear_command,
            category="tools",
            description="ارسال چند فحش تصادفی",
            usage="@swear [تعداد]",
            owner_only=True,
        )
    )
